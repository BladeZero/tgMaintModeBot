...
while is_started do -- Start a loop while the bot should be running.
	...
	if res then
		...
		for i,msg in ipairs(res.result) do -- Go through every new message.
			...
			if msg.message  then
				if msg.message.text then
					if msg.message.date > os.time() - 5 then
						local botname = "@werewolfbot"
						if string.match(msg.message.text, botname) then
							api.sendMessage(msg.message.chat.id, "This bot is currently under maintainence, please do not spam me, rather switch to @werewolfiivbot")
							if msg.message.from then 
								print("Notified of parking, "..msg.message.chat.id..', '..msg.message.from.id)
								print(msg.message.text)
							else
								print("Notified of parking, "..msg.message.chat.id)
								print(msg.message.text)
							end
						end
